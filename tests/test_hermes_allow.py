...
def test_is_match():
    ...
    assert_true(_is_match("podman ps --all --no-trunc", "podman ps*"), "podman ps many flags")
    ...
    assert_true(_is_match("podman run -it fedora", "podman run *"), "podman run with args")
    assert_true(_is_match("podman run --rm alpine sh", "podman run *"), "podman run --rm")
    assert_false(_is_match("podman run", "podman run *"), "podman run bare: no args, space required")
    ...
    assert_true(_is_match("podman run --rm -it fedora bash", "podman run *"), "podman run long")
...
def test_check_allow():
    ...
    for cmd in ["ls", "ls -la", "ls -la /tmp", "cat README.md", "head -20 file.txt",
                "tail -f log.txt", "wc -l file.txt", "pwd", "echo hello", "which git",
                "whoami", "uname -a", "date", "env", "printenv PATH"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- VCS ---
    for cmd in ["git status", "git log --oneline", "git diff HEAD",
                "git push origin main", "git pull", "git fetch --all",
                'git commit -m "fix: correct off-by-one"',
                'git commit -m "feat: support && operator in messages"',
                "gh pr list", "git-lfs status"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Rust toolchain ---
    for cmd in ["cargo build", "cargo test", "cargo check", "cargo fmt",
                "cargo clippy", "rustc --version", "rustup show", "rustfmt src/main.rs",
                "cargo-watch -x test"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Python toolchain ---
    for cmd in ["python3 script.py", "python3 -m pytest", "python3 -m mypy src/",
                "pytest tests/", "ruff check .", "ruff format .", "mypy src/",
                "black .", "isort .", "pip list", "pip3 show requests",
                "uv pip install -r requirements.txt"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Build tools ---
    for cmd in ["make", "make clean", "make test", "cmake ..", "just build",
                "task lint", "./build.sh", "./run_web.sh"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Search / text tools ---
    for cmd in ["grep -r 'TODO' src/", "rg 'fn main'", "find . -name '*.rs'",
                "sed 's/foo/bar/g' file.txt", "awk '{print $1}' file.txt",
                "sort file.txt", "diff a.txt b.txt", "jq '.name' data.json",
                "cat file.txt | head -10"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Network queries (read-only) ---
    for cmd in ["curl https://example.com", "wget https://example.com/file",
                "ping -c 3 8.8.8.8", "dig github.com"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- System info ---
    for cmd in ["du -sh .", "df -h", "free -m", "ps aux", "top -b -n1",
                "stat file.txt", "file binary", "sha256sum archive.tar.gz"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")
    ...
    for cmd in ["podman ps", "podman ps -a", "podman images", "podman images -a",
                "podman logs mycontainer", "podman logs -f mycontainer",
                "podman build -t myimage .", "podman run -it fedora bash",
                "podman run --rm alpine sh", "podman exec mycontainer ls",
                "podman inspect mycontainer", "podman top mycontainer",
                "podman stats --no-stream",
                "docker ps", "docker ps -a", "docker images",
                "docker logs mycontainer", "docker build -t myimage .",
                "docker run -it alpine sh", "docker exec mycontainer ls",
                "docker inspect mycontainer", "docker stats --no-stream"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Distrobox ---
    for cmd in ["distrobox-enter", "distrobox enter fedora-toolbox",
                "distrobox list", "distrobox list --all", "toolbox"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow: {cmd}")

    # --- Explicitly absent (intentionally blocked in default allow mode) ---
    for cmd in ["rm file.txt", "rm -rf /tmp/test", "mv a.txt b.txt",
                "chmod +x script.sh", "chown user:group file.txt",
                "ln -s src dst", "kill 1234", "killall python3",
                "ssh user@host", "scp file user@host:/tmp",
                "rsync -av src/ dst/",
                "tar czf archive.tar.gz dir/", "gzip file.txt",
                "zip archive.zip file.txt", "unzip archive.zip",
                "vim file.txt", "nano file.txt", "hx file.txt",
                "strace ls", "gdb ./binary"]:
        result = _check_allow(cmd, _DEFAULT_ALLOWED)
        assert_not_none(result, f"should block: {cmd}")
        assert_in("not in allowed list", result), f"error message format wrong for: {cmd}"

    # --- Dangerous container commands blocked in allow mode ---
    for cmd in ["podman system prune -a", "podman system reset",
                "podman rm mycontainer", "podman rmi myimage",
                "docker system prune", "docker rm mycontainer", "docker rmi myimage"]:
        result = _check_allow(cmd, _DEFAULT_ALLOWED)
        assert_not_none(result, f"should block: {cmd}")
    ...
    for cmd in ["git status && git log", "ls -la; pwd",
                "cat file.txt | head -10",
                "git status || echo 'not a git repo'",
                "grep -r TODO . | sort | uniq"]:
        assert_none(_check_allow(cmd, _DEFAULT_ALLOWED), f"should allow compound: {cmd}")

    # --- Compound commands: one blocked segment blocks the whole command ---
    for cmd in ["rm -rf test && cat test", "cat test; rm test",
...
def test_check_block():
    ...
    for cmd in ["git status", "git log", "ls -la", "cat README.md",
                "python3 script.py", "cargo build", "ruff check .",
                "curl https://example.com", "grep -r TODO .",
                "podman ps", "podman ps -a", "podman images",
                "podman logs mycontainer", "podman run -it fedora bash",
                "docker ps", "docker images", "docker run -it alpine sh",
                "rm file.txt", "rm -r testdir", "rm -f file.txt",
                "vim file.txt", "ssh user@host"]:
        assert_none(_check_block(cmd, _DEFAULT_BLOCKED), f"should allow: {cmd}")

    # --- rm -rf / rm -fr variants ---
    for cmd in ["rm -rf /tmp/test", "rm -fr /tmp/test",
    ...
    for cmd in ["podman prune -a", "podman system prune -a",
    ...
    for cmd in ["podman system reset", "podman system info",
                "docker system df", "docker system events"]:
        result = _check_block(cmd, _DEFAULT_BLOCKED)
        assert_not_none(result, f"should block: {cmd}")

    # --- Container remove commands ---
    for cmd in ["podman rm mycontainer", "podman rm -f mycontainer",
                "podman rm container1 container2",
                "podman rmi myimage", "podman rmi myimage:latest",
                "docker rm mycontainer", "docker rm -f c1 c2",
                "docker rmi myimage", "docker rmi myimage:v1"]:
        result = _check_block(cmd, _DEFAULT_BLOCKED)
        assert_not_none(result, f"should block: {cmd}")
    ...
    for cmd in ["git status && rm -rf /tmp/test",
    ...
    for cmd in ["git status && git log", "ls -la; pwd",
                "cat file.txt | grep TODO", "rm file.txt; git status"]:
        assert_none(_check_block(cmd, _DEFAULT_BLOCKED), f"should allow compound: {cmd}")
